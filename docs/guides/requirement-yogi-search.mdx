...
| `AND` | Both conditions must match | `key ~ 'AS_%' AND @Priority = 'High'` |
...
|-------|-------------|---------|
| `key` | Requirement key | `key = 'AS_001'` or `key ~ 'AS_%'` |
| `spaceKey` | Confluence space key (case-sensitive) | `spaceKey = 'OCX'` |
...
| Field | Description | Example |
|-------|-------------|---------|
| `jira` | Linked Jira issue key | `jira = 'OCX-11076'` |
| `jira@relationship` | Jira issue with a specific relationship type | `jira@implements = 'OCX-11076'` |
...
2. **Prefer key patterns over text** — `key ~ 'AS_%'` is faster than `text ~ '%AS%'`
